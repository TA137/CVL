...
<div class="content-box-large">
				  ...
				  <?php
									  $stmt = $DB_con->prepare("SELECT *,upload_doc.id id_doc FROM upload_doc join organization join categories on organization.id=upload_doc.org_id and upload_doc.cat_id=categories.id where upload_doc.id=(select upload_id from upload_subcategories where org_id=:org_id and cat_id=:cat_id and sub_cat_id=:sub_cat)");
									  $count_stmt = $DB_con->prepare("SELECT count(*) apl_num FROM upload_doc join organization join categories on organization.id=upload_doc.org_id and upload_doc.cat_id=categories.id where upload_doc.id=(select upload_id from upload_subcategories where org_id=:org_id and cat_id=:cat_id and sub_cat_id=:sub_cat)");
									  $stmt->execute(array('org_id'=>$organization_id,':cat_id'=>$cat_id,':sub_cat'=>$sub_cat_id));
									  $count_stmt->execute(array('org_id'=>$organization_id,':cat_id'=>$cat_id,':sub_cat'=>$sub_cat_id));
